...
zip=$(basename $crpath/out/target/product/$device/crDroidAndroid-12.1-*-$device-*.zip)        # Here we get the package name with the extension .zip
nozip=$(basename $crpath/out/target/product/$device/crDroidAndroid-12.1-*-$device-*.zip .zip) # Here we get the package name without the extension .zip
...
gapps="https://sourceforge.net/projects/nikgapps/files/Releases/Android-12.1/31-Dec-2024/NikGapps-core-arm64-12.1-20241231-signed.zip/download"
...
gh release create $nozip --notes "Automated release сrDroid for $device $version $date/$time" $crpath/out/target/product/$device/$zip
