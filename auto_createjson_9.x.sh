...
zip=$(basename out/target/product/$device/crDroidAndroid-13.0-*-$device-*.zip)        # Here we get the package name with the extension .zip
nozip=$(basename out/target/product/$device/crDroidAndroid-13.0-*-$device-*.zip .zip) # Here we get the package name without the extension .zip
...
rm -rf 9.x/$device.json && cp ~/android/crdroid9/$device.json 9.x
